...
can use the `read.csv()` function from the **`tidyverse`** package.
...
dataset, `read.csv()` reads `weight` as `col_double` (a numeric data
...
`read.csv`. <note> not sure this is what it does in read.csv</note>
...
<note> Can just skip this but leave in notes for others to read later </note>
...
plot(surveys$sex)
```
<note> Check whether this is necessary, it seems needlessly complex</note>
