...
<!-- Create a cleaned data set and place it in the download folders so that people 
who are lost can just load the data set -->
...
```{r}
surveys_complete <- read_csv('data_raw/portal_surveys_complete.csv')

```

For those of yall who werent able to create the file from the last lesson. 
You can grab it here. But make sure to go back after the workshop and try to recreate the file correctly later.
...
Looking at this plot, its not too bad. What are ways in which we can make this plot better?
Ideas:  
- Thicker lines
...
### Extra innings questions

What other kinds of plots would people like to see?

Do you want to work through an ugly plot together and fix it?

Would you prefer to move forward and go to more advanced subjects?
...
## Fix a bad graph either together or as a challenge
