...
R is widely used in academia and very heavily in the wildlife field.
Any analysis even common ones, have packages specifically for them in R.
Employees will expect you know how to do these analysis and use these programs.
...
Right now I want you to create 2 directories (folders) calle **scripts** and **data**.
...
For this workshop, we will need `data/` for when we learn how to export data as CSV files, and a `fig/` folder for the figures that we will save.

- Under the `Files` tab on the right of the screen, click on `New Folder` and
  create a folder named `data` within your newly created working directory
  (e.g., `~/data-carpentry/`). (Alternatively, type `dir.create("data")`
  at your R console.) Repeat these operations to create a `fig`
  folder.
...
# Just using the help pane. To me it also confuses people a lot
# </note>
...
through the lessons after we're done here. And use that time to try things out
that you didnt have time during class.  
