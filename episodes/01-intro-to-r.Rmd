...
a peanut butter jelly sandwich and gaurantee they'll make it the same everytime?

Well you can write a recipe for them, or you can just build them a
'peanut butter and jelly making machine'. In R we have these same options. We can either
share scripts or make them functions/packages. except R makes making a pbj machine much easier. 

So lets start with writing a recipe. How would you write a recipe for them and share that with them. It would look something like this:
...
- Place the two pieces of bread together (peanut butter to jelly)
- Cut diagonal with a knife
- Serve!
```

I can hand you this recipe and you'll know how to make a pb&j sandwich reliably. 
This is your script. The script itself does not do the work. The script is just the recipe you hand over. The console is *you*, *you* are the one making the recipe.

We'll play with this concept as we go.

Now whats a function? Well its basically a pb&j machine. It uses a recipe. Takes the required ingredients. Makes your sandwich. Poops it out. Makes the same sandwich every single time.
...
It has two arguments, x and digits. These are the ingredients to your round sandwich. x is a common variable term for the data, in order to calculate the standard deviation it obviously needs *the data*. 

It also is telling us it needs something else, digits. I know from this and from the help file, this argument is telling us how many digits to round our number to. This digits argument is set equal to 0. What does that mean? When youre running a function, the = means youre assigning a value to that function. This is how we tell the function what ingredient we want to use. Maybe you want to give it wheat bread, not white bread, maybe grape jelly not strawberry. etc.

In our pb&j function we're essentially doing this:

```
pbj(bread = "white bread", jelly="smuckers grape jelly", peanut_butter = "jif smooth peanut butter")
```

So what this function is trying to tell us with digits=0 is basically telling us its default answer. If we don't tell it what kind of peanut butter or jelly we want to use, it will use the default.

We can rewrite our pbj recipe like this:
```
Peanut Butter and Jelly Sandwich
Ingredients
- 2 slices of sandwich bread
- Jelly (Smuckers Grape Jelly)
- Peanut butter (Jif Smooth Peanut Butter)
...
And in our SD function, its telling us if we dont assign a value to digits its going to use its default '0' meaning round to 0 decimals.
</note>
...
<note> 
We usually lose some people here. Maybe pause and talk about what a character is and why its important to specify it. Also go over that you can use single or double quotes.
</note>

<note>
In all programming languages you have to specify the kinds of data youre using. 
In R all R objects are built mainly from 4 **data types or atomic vector** :
- `"numeric"` 
- `"character"` 
- `"logical"` for `TRUE` and `FALSE` (the boolean data type)
- `"integer"` for integer numbers. Essentially a numeric with no decimal.
- `"complex" and "raw"` - more complex and not in the scope of this lesson.
...
Programming languages largely dont like to guess, and they shouldnt, if they do
it should be well documented what and why. The easiest and safest way to write
languages is to have these standards. Having these data types is a way of making
sure there is a standard to help R run smoothyl and reliably everytime.

This easiest example is '1'. 

'1' can be all 4 data types

1 is a number. Its the first number!. It can also have a certain accuracy
1 is also an integer. Its a non-decimal exact number. Its 1!
1 is also used in logical. Well known in programming, but less known outside. 
...
1 can also be a character. "1" its not the first its not the measured value of 1. It is simply maybe your first plot. Number 1 because maybe an arbitrary reason. It was the first plot you went to.

Now if youre trying to make a recipe and you gave it a value of 1, you can argue that
the first three examples are sort of the same and shouldnt matter. Whether it was
a numeric, integer, or logical representation of 1, you should always calculate the standard deviation, square root, mean, should always be the same. Logical might mean
youre doing something wrong, however. As that doesnt make *that much sense*. 
But what if 1 really meant 'Plot 1', in that case taking the mean of 'Plot 1' 
well...is very illogical. Youd want it to probably stop and warn you something is weird. And thats actually what R does!. Another reason to have these data standards.
...
:::::::::::::::::::::::::::::::::::::::  challenge
# <note> we need another challenge here. Do an actual challenge and then at the
# end have an option challenge
# </note>
