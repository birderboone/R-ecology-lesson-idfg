...
<notes> Use this time to explain and show that actually means to assign something a value </notes>
...
(weight_kg <- 55)  # less often used, but parenthesis in code will also print the output
...
share scripts or make them functions/packages. Except in R making a PB&J machine is pretty easy compared to real life.
...
Now whats a function? Well its basically a PB&J machine. It uses a recipe. Takes the required ingredients you give it. Makes your sandwich. Pops it out. It makes the same sandwich every single time.
...
In our theoretical PB&J function this is what our arguments would be:

```
pbj(bread = "white bread", jelly="smuckers grape jelly", peanut_butter = "jif smooth peanut butter")
```

So what the round function is trying to tell us with digits=0 is it's telling us the default answer its going to use if you dont give it one. If we don't tell it what kind of peanut butter or jelly we want to use, it will use the default: JIF smooth peanut butter and Scmuckers grape jelly.

We can rewrite our PB&J recipe to match this concept like this:
```
Peanut Butter and Jelly Sandwich
Ingredients
- 2 slices of sandwich bread
...
And in our round function, its telling us if we don't assign a value to `digits` its going to use its default '0' meaning round to 0 decimals.
...
**\<note>** I've heard the rule of thumb to name your arguments if its more than 3 **\</note>**
...
- 1 can also be a character. "1" its not the first its not the measured value of 1.  
...
### Challenge
<!-- rewrite this challenge to test how to subset vectors and use the %in% function then ask the bonus challege -->
...
<!--  we need another challenge here. Do an actual challenge and then at the
